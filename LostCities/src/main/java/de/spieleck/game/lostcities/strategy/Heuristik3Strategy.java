...
public class Heuristik3Strategy
    implements Strategy
{
    ...
    private int score(Perspective p2) {
        ...
        int zeros = 0;
        for(int i : cnt) if ( i == 0) zeros++;
        score -= 3 * zeros*zeros;
        // score += 1 * valueSum(hand, p2.pileSize()/9);
        score -= 6 * valueSum(p2.discards(), 5);
        // score -= p2.pileSize() * p2.pileSize() * 6; // we are superior, we play fast
        score -= 250 * p2.pileSize(); // we are superior, we play fast
        return score;
    }
